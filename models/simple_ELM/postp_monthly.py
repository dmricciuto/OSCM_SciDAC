...
for iqoi in range(nqois):
    ...
    for site_id, lon_id, lat_id in site_lon_lat:
        ...
        aa = dataset.variables[qoi][
            :, 0, :, lat_id, lon_id].reshape(nens, -1, twelve)
        ytrain[:, jout:jout + twelve] = np.average(aa, axis=1)
        ytrain[:, jout + twelve:jout + 2 * twelve] = np.std(aa, axis=1)

        if get_obsdata:
            var_daily = obs_dataset.variables[
                qoi.upper()][site_id, :] * 24 * 3600 * 1000
            var_monthly = np.nanmean(daily_to_monthly(
                var_daily).reshape(24, twelve), axis=0)
            ydata = np.append(ydata, var_monthly)
        for i in range(twelve):
            xdata = np.append(
                xdata, [[lat_id, lon_id, iqoi, i + 1, 0]], axis=0)
            outnames.append(qoi + '_' + str(lat_id) + '_' +
                            str(lon_id) + '_m ' + monthnames[i])
        for i in range(twelve):
            xdata = np.append(
                xdata, [[lat_id, lon_id, iqoi, i + 1, 1]], axis=0)
            #ydata   = np.append(ydata, -999.)
            outnames.append(qoi + '_' + str(lat_id) + '_' +
                            str(lon_id) + '_s ' + monthnames[i])
